...
class DataIngestion:
    ...
    def get_data_from_gcloud(self) -> None:
        try:
            ...
            data_file = os.path.join(self.data_ingestion_config.DATA_INGESTION_ARTIFACTS_DIR,self.data_ingestion_config.ZIP_FILE_NAME)
            if not os.path.isfile(data_file):
                self.gcloud.sync_folder_from_gcloud(self.data_ingestion_config.BUCKET_NAME,
                                                #self.data_ingestion_config.ZIP_FILE_NAME, #dont need because downloading from url
                                                data_file, #downlaod datafile path
